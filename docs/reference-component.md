...
| `colorScheme` | `'light' \| 'dark' \| 'auto'` | `'light'` | Dark mode. Adds `note-dark` to the root; all four skins theme their chrome from CSS variables. `'auto'` follows the OS `prefers-color-scheme`. See [Dark mode](./reference-options.md#dark-mode). |

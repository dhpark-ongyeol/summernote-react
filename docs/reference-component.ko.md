...
| `colorScheme` | `'light' \| 'dark' \| 'auto'` | `'light'` | 다크 모드입니다. 루트에 `note-dark`를 추가하며, 네 스킨 모두 CSS 변수로 chrome을 테마링합니다. `'auto'`는 OS `prefers-color-scheme`를 따릅니다. [Dark mode](./reference-options.md#dark-mode)를 참고하세요. |
